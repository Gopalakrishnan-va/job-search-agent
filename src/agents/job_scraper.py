...
class JobScraperAgent(BaseAgent):
    ...
    async def process(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for job in linkedin_jobs + indeed_jobs:
            score = self._calculate_initial_score(job, resume_data)
            job["initial_score"] = score
            all_jobs.append(job)
        ...
        job_urls = [job["url"] for job in promising_jobs]
    ...
    def _score_location_match(self, job_location: str, preferred_location: str) -> float:
        ...
        if "remote" in job_location.lower():
            return 1.0
        elif preferred_location.lower() in job_location.lower():
            return 1.0
